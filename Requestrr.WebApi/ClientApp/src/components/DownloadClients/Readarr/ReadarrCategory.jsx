...
import { loadReadarrProfiles as loadProfiles } from "../../../store/actions/ReadarrClientActions"
import { loadReadarrMetadataProfiles as loadMetadataProfiles } from "../../../store/actions/ReadarrClientActions"
import { loadReadarrRootPaths as loadRootPaths } from "../../../store/actions/ReadarrClientActions"
import { loadReadarrTags as loadTags } from "../../../store/actions/ReadarrClientActions"
import { setReadarrCategory } from "../../../store/actions/ReadarrClientActions"
import { removeReadarrCategory } from "../../../store/actions/ReadarrClientActions"
...
function ReadarrCategory(props) {
    ...
    const reduxState = useSelector((state) => {
        return {
            readarr: state.books.readarr,
            otherCategories: state.books.otherCategories
        }
    });
    ...
    useEffect(() => {
        ...
        let previousNames = prevState === undefined ? [] : prevState.readarr.categories.map(x => x.name);
        let currentNames = reduxState.readarr.categories.map(x => x.name);
    ...
    const validateName = (value) => {
        ...
        if (!/\S/.test(value)) {
            ...
            if (reduxState.readarr.categories.map(x => x.id).includes(props.category.id) && reduxState.readarr.categories.filter(c => typeof c.id !== 'undefined' && c.id !== props.category.id && c.name.toLowerCase().trim() === value.toLowerCase().trim()).length > 0) {
    ...
    const setCategory = (fieldChanged, data) => {
        dispatch(setReadarrCategory(props.category.id, fieldChanged, data));
    };


    const deleteCategory = () => {
        setIsOpen(false);
        setTimeout(() => dispatch(removeReadarrCategory(props.category.id), 150));
    };
    ...
    return (
        <>
            <tr class="fade show">
                ...
                <td class="text-right">
                    <button onClick={() => setIsOpen(!isOpen)} disabled={isOpen && (!isNameValid || !reduxState.readarr.arePathsValid || !reduxState.readarr.areProfilesValid || !reduxState.readarr.areTagsValid)} className="btn btn-icon btn-3 btn-info" type="button">
                ...
                <td className="border-0 pb-0 pt-0" colSpan="2">
                    <Collapse isOpen={isOpen}>
                        <div className="mb-4">
                                ...
                                <Col lg="6" className="mb-4">
                                    <div className="input-group-button">
                                        <Dropdown
                                            ...
                                            items={reduxState.readarr.paths.map(x => { return { name: x.path, value: x.path } })}
                                        ...
                                        <button className="btn btn-icon btn-3 btn-default" onClick={() => dispatch(loadRootPaths(true))} disabled={!props.canConnect} type="button">
                                            <span className="btn-inner--icon">
                                                {
                                                    reduxState.readarr.isLoadingPaths ? (
                                                        <Oval
                                                            wrapperClass="loader"
                                                            type="Oval"
                                                            color="#11cdef"
                                    ...
                                    {
                                        !reduxState.readarr.arePathsValid ? (
                                    ...
                                    {
                                        props.isSubmitted && reduxState.readarr.paths.length === 0 ? (
                                            <Alert className="mt-3 mb-0 text-wrap " color="warning">
                                ...
                                <Col lg="6" className="mb-4">
                                    <div className="input-group-button">
                                        <Dropdown
                                            ...
                                            items={reduxState.readarr.profiles.map(x => { return { name: x.name, value: x.id } })}
                                        ...
                                        <button className="btn btn-icon btn-3 btn-default" onClick={() => dispatch(loadProfiles(true))} disabled={!props.canConnect} type="button">
                                            <span className="btn-inner--icon">
                                                {
                                                    reduxState.readarr.isLoadingProfiles ? (
                                                        <Oval
                                                            wrapperClass="loader"
                                                            type="Oval"
                                                            color="#11cdef"
                                    ...
                                    {
                                        !reduxState.readarr.areProfilesValid ? (
                                            <Alert className="mt-3 mb-0 text-wrap " color="warning">
                                    ...
                                    {
                                        props.isSubmitted && reduxState.readarr.profiles.length === 0 ? (
                                            <Alert className="mt-3 mb-0 text-wrap " color="warning">
                                ...
                                <Col lg="6">
                                    <div className="input-group-button mb-4">
                                        <MultiDropdown
                                            ...
                                            selectedItems={reduxState.readarr.tags.filter(x => props.category.tags.includes(x.id))}
                                            items={reduxState.readarr.tags}
                                        ...
                                        <button className="btn btn-icon btn-3 btn-default" onClick={() => dispatch(loadTags(true))} disabled={!props.canConnect} type="button">
                                            <span className="btn-inner--icon">
                                                {
                                                    reduxState.readarr.isLoadingTags ? (
                                                        <Oval
                                                            wrapperClass="loader"
                                                            type="Oval"
                                                            color="#11cdef"
                                    ...
                                    {
                                        !reduxState.readarr.areTagsValid ? (
                                            <Alert className="mt-3 mb-4 text-wrap " color="warning">
                                                <strong>Could not load tags, cannot reach Readarr.</strong>
                                            </Alert>)
                                            : null
                                    }
                                </Col>
                                <Col lg="6" className="mb-4">
                                    <div className="input-group-button">
                                        <Dropdown
                                            ...
                                            items={reduxState.readarr.metadataProfiles.map(x => { return { name: x.name, value: x.id } })}
                                        ...
                                        <button className="btn btn-icon btn-3 btn-default" onClick={() => dispatch(loadMetadataProfiles(true))} disabled={!props.canConnect} type="button">
                                            <span className="btn-inner--icon">
                                                {
                                                    reduxState.readarr.isLoadingMetadataProfiles ? (
                                                        <Oval
                                                            wrapperClass="loader"
                                                            type="Oval"
                                                            color="#11cdef"
                                    ...
                                    {
                                        !reduxState.readarr.areMetadataProfilesValid ? (
                                            <Alert className="mt-3 mb-0 text-wrap " color="warning">
                                    ...
                                    {
                                        props.isSubmitted && reduxState.readarr.metadataProfiles.length === 0 ? (
                                            <Alert className="mt-3 mb-0 text-wrap " color="warning">
                            ...
                            {
                                reduxState.readarr.categories.length > 1
                                    ? <Row>
...
export default ReadarrCategory;
